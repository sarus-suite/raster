...
pub trait SarusMountsTrait {
    ...
    #[allow(unused_variables)]
    fn from(&mut self, input: Vec<String>) -> SarusResult<()> {
         Ok(())
    }   
}
        ...
        if asize < 2 || asize > 3 {
            return Err(
                SarusError {
                    code: 8,
                    file_path: None,
                    msg: format!("{} contains {} number of fields, expected 2 or 3", input, asize),
                });
        };
